	...
	<BODY>
		...
		<main id="body" class="col-lg-12 nopadding">
				...
				<div class="col-lg-12 row nopadding jumbotron" style="margin-top: 1.5em;">
				  <h1 class="text-center"> <img src="../../Lib/img/catalogo/logo_bmw.png" alt="Logo BMW"> </h1>
				</div>
			<!-- Imagenes -->
				...
				<?php 
					...
					$array = $micoche->dame_coches("BMW");
					
		            for($i=0; $i< count($array); $i+=1){
		            	$coche1 = $array[$i];
		            	$modeloc1 = str_replace(" ","-",$coche1->get_modelo());
		            	$carritoCoche1 = htmlspecialchars("btn_add_carrito('".$coche1->get_id()."','".$coche1->get_precio()."','".$coche1->get_marca()."','".$modeloc1."','".$coche1->get_foto_miniatura()."');", ENT_QUOTES);
						...
						echo '	      <p>';
						echo '		<button type="button" class="btn btn-custom" onclick='.$carritoCoche1.'><i class="fa fa-cart-plus"></i> Añadir al Carro</button>';
						echo '		<a href="detalles_coche?id='.$coche1->get_id().'&boton='.$carritoCoche1.'" class="btn btn-info" ><i class="fa fa-info"></i> Detalles </a>';
		            	...
		            	$carritoCoche2 = htmlspecialchars("btn_add_carrito('".$coche2->get_id()."','".$coche2->get_precio()."','".$coche2->get_marca()."','".$modeloc2."','".$coche2->get_foto_miniatura()."');", ENT_QUOTES);
							...
							echo '	      <p>';
							echo '		<button type="button" class="btn btn-custom" onclick='.$carritoCoche2.'><i class="fa fa-cart-plus"></i> Añadir al Carro</button>';
							echo '		<a href="detalles_coche?id='.$coche2->get_id().'&boton='.$carritoCoche2.'" class="btn btn-info" ><i class="fa fa-info"></i> Detalles </a>';
							echo '		<a href="#" class="btn btn-default" data-toggle="modal" data-target="#myModal"><i class="fa fa-share-alt"></i> Compartir</a>';
							echo '	      </p>';
							echo '	   </div>';
							echo '</div>';
